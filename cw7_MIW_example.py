...
# grupowanie = k_nn(matrix, len(matrix[0])-1, [1 for x in range(len(matrix[0]))])
# grupowanie[1].sort()
# grupowanie[2].sort()
...
# grupy = grupujemy(k_nn_lista(matrix, len(matrix[0])-1, [1 for x in range(len(matrix[0]))]),5)
...
def nauczyciel(lista):
    ...
    for ele in lista:
        ...
        if decyzyjna in grupy.keys():
            ...
            grupy[decyzyjna]=[ele]
    matrix_without_decission = [l[:-1]+[float(grupy.keys()[randrange(len(grupy.keys()))])] for l in lista]
...
def matrix_difference(m1,m2):
    ...
    for i in range(len(m1)):
        if(m1[i][-1]!=m2[i][-1]):
            ile=ile+1
    return str((len(m1)-ile)/len(m1)*100)+" % pokrycia w stosunku do oryginalu"
    
new_matrix = nauczyciel(copy.deepcopy(matrix))
